...
import emailRegistro from '../helpers/emailResgistro.js';
...
const registrar = async (req,res) => {
    ...
    if(existeUsuario){
        const error = new Error('Usuario  ya fue registrado');
    ...
    res.json({msg: "Registrado usuario..."});
};
...
const comprobarToken = async (req,res) => {
    ...
    if(tokenValido){
        //El toke es valido el usuario existe
        res.json({msg:'Token Valido y el usuario existe'});
    }else{
        const error = new Error('Token no valído');
        return res.status(400).json({msg: error.message});
    }
